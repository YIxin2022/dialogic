...
func add_portrait_group() -> void:
	...
	var item: TreeItem = %PortraitTree.add_portrait_group("组", parent_item)
	item.set_meta('new', true)
	item.set_editable(0, true)
	item.select(0)
	%PortraitTree.call_deferred('edit_selected')
...
func _on_fit_preview_toggle_toggled(button_pressed):
	...
	if button_pressed:
		...
		%FitPreview_Toggle.tooltip_text = "实际尺寸"
	else:
		%FitPreview_Toggle.tooltip_text = "适展预览"
