...
func _ready() -> void:
	...
	for i in %Filters.get_children():
		...
		# In Godot 4.x, the editor's .tscn connection does not pass the button reference 
		# unless explicitly bound, so we bind it in _ready and make sure it doesn't double-connect
		if not i.toggled.is_connected(_on_filter_toggled):
			i.toggled.connect(_on_filter_toggled.bind(i))
...
func _on_collapse_toggled(button_pressed:bool) -> void:
	...
	if button_pressed:
		...
		%Collapse.tooltip_text = "展开全部"
	else:
		%Collapse.icon = get_theme_icon("CollapseTree", "EditorIcons")
		%Collapse.tooltip_text = "折叠全部"
...
func load_modules_tree() -> void:
	...
	for i in indexers:
		...
		var module_name: String = str(i.get_script().resource_path.trim_suffix("/index.gd").get_file())
		var mod_trans := {
			...
			"DefaultLayoutParts": "默认布局部件 (DefaultLayoutParts)",
			"StyleEditor": "样式编辑器 (StyleEditor)",
			"Theme": "主题 (Theme)",
			"Layout": "布局 (Layout)",
			"Screen": "屏幕 (Screen)",
		...
		module_item.set_text(0, module_name)
		...
		for ev in i._get_events():
			...
			for cached_event in cached_events:
				if cached_event.get_script().resource_path == ev:
					event_item.set_text(0, cached_event.event_name + " 事件")
					...
					event_item.add_button(0, get_theme_icon("GuiVisibilityVisible", "EditorIcons"), 0, false, "切换事件按鈕可视性")
					if is_hidden:
...
func _on_tree_item_selected() -> void:
	...
	if metadata is Dictionary:
		match metadata.type:
			"Event":
				%GeneralInfo.text = "事件可在时间轴中使用，能完成各种操作。它们通常与子系统和 Dialogic 节点交互。"
			...
			# -------------------------------------------------
			"Subsystem":
				%GeneralInfo.text = "子系统提供专用功能。它们主要管理事件与 Dialogic 节点之间的通信。通常它们提供可以这样访问的局机方法：Dialogic.Subsystem.a_method()。"
			# -------------------------------------------------
			"Effect":
				%GeneralInfo.text = "文本效果可在文本事件中使用。到达时执行，可接受单个参数。"
				%ShowInFileSystem.disabled = true
				%Open.disabled = true
			# -------------------------------------------------
			"Modifier":
				%GeneralInfo.text = "修饰器可在显示前修改文本事件中的文本。"
				%ShowInFileSystem.disabled = true
				%Open.disabled = true
			# -------------------------------------------------
			"Style":
				%GeneralInfo.text = "样式预设可在样式编辑器中激活和修改。它们提供游戏中对话界面的设计。"
			# -------------------------------------------------
			"Editor":
				%GeneralInfo.text = "编辑器提供编辑 Dialogic 数据的用户界面。"
			# -------------------------------------------------
			"Settings":
				%GeneralInfo.text = "设置页面提供子系统、事件和 Dialogic 节点通常使用的设置。"
			"Module":
				%GeneralInfo.text = ""
				%Open.disabled = true
			# -------------------------------------------------
