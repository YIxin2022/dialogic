...
func _ready() -> void:
	...
	%UpdateConfirmationDialog.add_button("保留旧内容并生成新内容", false, "generate_new")
...
func update_csv_files() -> void:
	...
	var status_message := "事件    已创建 {new_events}   已找到 {updated_events}
		名称  已创建 {new_names}   已找到 {updated_names}
		CSV      已创建 {new_timelines}   已找到 {updated_timelines}
		词汇表  已创建 {new_glossaries}   已找到 {updated_glossaries}
		条目   已创建 {new_glossary_entries}   已找到 {updated_glossary_entries}"
...
func collect_translations() -> void:
	...
	%StatusMessage.text = (
		"已添加翻译文件：" + str(len(all_translation_files)-orig_file_amount)
		+ "\n已移除翻译文件：" + str(removed_translation_files)
...
func erase_translations() -> void:
	...
	var status_message := "时间轴已清理 {cleaned_timelines}
		事件已清理 {cleaned_events}
		角色已清理 {cleaned_characters}
		词汇表已清理 {cleaned_glossaries}
