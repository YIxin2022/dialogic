...
func reload_section_list():
	...
	for sec in sections:
		...
		var display_name: String = str(sec)
		if display_name == "Main": display_name = "Main (文本)"
		elif display_name == "Flow": display_name = "Flow (控制流)"
		elif display_name == "Audio": display_name = "Audio (音频)"
		elif display_name == "Visuals": display_name = "Visuals (视觉)"
		elif display_name == "Logic": display_name = "Logic (逻辑)"
		elif display_name == "Other": display_name = "Other (其他)"
		
		item.set_metadata(0, sec)
		item.set_text(0, display_name + (" (空)" if empty_category else ""))
...
func _on_section_list_button_clicked(item:TreeItem, _column, id, _mouse_button_index):
	...
	for child in %SectionList.get_root().get_children():
		if child.get_metadata(0):
			sections.append(child.get_metadata(0))
		else:
			sections.append(child.get_text(0).split(" ")[0])
