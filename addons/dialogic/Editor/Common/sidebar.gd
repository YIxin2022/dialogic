...
func _ready() -> void:
	...
	grouping_menu.add_icon_radio_check_item(get_theme_icon("AnimationTrackList", "EditorIcons"), "无", 0)
	grouping_menu.add_icon_radio_check_item(get_theme_icon("Folder", "EditorIcons"), "按类型", 1)
	grouping_menu.add_icon_radio_check_item(get_theme_icon("FolderBrowse", "EditorIcons"), "按文件夹", 2)
	grouping_menu.add_icon_radio_check_item(get_theme_icon("AnimationTrackGroup", "EditorIcons"), "按路径", 3)

	var options_popup: PopupMenu = %Options.get_popup()
	options_popup.hide_on_checkable_item_selection = false
	options_popup.add_submenu_node_item("分组方式", grouping_menu)
	options_popup.add_check_item("使用文件夹颜色", 11)
	options_popup.add_check_item("省略文件夹路径", 12)
	options_popup.add_item("列出全部", 21)
	...
	%RightClickItemMenu.add_icon_item(get_theme_icon("Remove", "EditorIcons"), "从列表移除", 1)
	%RightClickItemMenu.add_separator()
	%RightClickItemMenu.add_icon_item(get_theme_icon("ActionCopy", "EditorIcons"), "复制标识符", 4)
	%RightClickItemMenu.add_separator()
	%RightClickItemMenu.add_icon_item(
		get_theme_icon("ShowInFileSystem", "EditorIcons"), "在文件系统中显示", 2
	)
	%RightClickItemMenu.add_icon_item(
		get_theme_icon("ExternalLink", "EditorIcons"), "在外部程序中打开", 3
	)
	...
	match group_mode:
		...
		GroupMode.TYPE:
			...
			if character_items.size() > 0:
				var character_tree := add_folder_item("角色", root)
				for item in character_items:
					add_item(item, character_tree, current_file)

			if timeline_items.size() > 0:
				var timeline_tree := add_folder_item("时间轴", root)
