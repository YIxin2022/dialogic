...
func _init() -> void:
	event_name = "角色"
	event_description = "允许角色加入或离开，或更新其肖像、位置、镜像、Z 槃索引或动画。"
...
func build_event_editor() -> void:
	...
	add_header_edit('set_portrait', ValueType.BOOL_BUTTON,
			{'icon':load("res://addons/dialogic/Modules/Character/update_portrait.svg"),
			 'tooltip':'更改肖像'}, "should_show_portrait_selector() and action == Actions.UPDATE")
	...
	add_header_label('位于', 'character != null and !has_no_portraits() and action == Actions.JOIN')
	...
	add_body_edit('repeat_forever', ValueType.BOOL, {'left_text':'无限循环:'},
			'should_show_animation_options() and !animation_name.is_empty() and action == %s)' %Actions.UPDATE)
	add_body_edit('animation_repeats', ValueType.NUMBER, {'left_text':'重复:', 'mode':1, "min":1},
	...
	add_body_line_break()
	add_body_edit('transform_time', ValueType.NUMBER, {'left_text':'移动时长:', "min":0, "tooltip": "When changing the characters position, this is how fast it will happen."},
			"should_show_transform_options()")
	add_body_edit("transform_trans", ValueType.FIXED_OPTIONS, {'options':trans_options, 'left_text':"过渡:", "tooltip":"The transition type to use for moving the character to its new position."}, 'should_show_transform_options() and transform_time > 0')
	add_body_edit("transform_ease", ValueType.FIXED_OPTIONS, {'options':ease_options, 'left_text':"缓动:", "tooltip":"The easing to use for moving the character to its new position."}, 'should_show_transform_options() and transform_time > 0')
	...
	add_body_edit('z_index', ValueType.NUMBER, {'left_text':'层级:', 'mode':1, "tooltip": "The Z-Index controls the visual order of characters. Higher z-index makes a character appear further in front."},
	...
	add_body_edit('mirrored', ValueType.BOOL, {'left_text':'水平翻转:', "tooltip": "Mirrors the character. This applies on top of the mirroring of the portrait and the position container."},
			'action != %s and (action != Actions.UPDATE or set_mirrored)' %Actions.LEAVE)
	add_body_edit('extra_data', ValueType.SINGLELINE_TEXT, {'left_text':'额外数据:', "tooltip": "Data that is given to the portrait. To be used on custom portrait scenes."}, 'action != Actions.LEAVE')
