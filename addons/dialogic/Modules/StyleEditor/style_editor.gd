...
func _register() -> void:
	...
	alternative_text = "自由更改和打扮您游戏内对话系统的皮肤布局视觉反馈"
...
func load_style(style:DialogicStyle) -> void:
	...
	if style.resource_path == default_style:
		%MakeDefaultButton.tooltip_text = "当前已被尊奉为标杆默认项"
		%MakeDefaultButton.disabled = true
	else:
		%MakeDefaultButton.button_pressed = false
		%MakeDefaultButton.tooltip_text = "设为全局默认生效皮肤"
	...
	if %InheritanceButton.visible:
		%InheritanceButton.text = "源自继承 " + style.inherits.name
